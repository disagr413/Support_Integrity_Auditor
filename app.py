...
with tab1:
    ...
    if st.button("🔍 Analyse Ticket", type="primary"):
        ...
        if score >= 0.35:
            pred = 1
            inf, mtype = get_verdict(row, max(prob, THR))
        ...
        if pred == 1:
            ...
            col_b.metric("Confidence", f"{max(prob, min(abs(score), 1.0)):.1%}")
            col_c.metric("Inferred Severity", inf, f"Assigned: {priority}")
        
            st.error(
                f"**{mtype}** — Assigned `{priority}` but model infers `{inf}`"
            )
            ...
            st.success(
                f"Priority **{priority}** appears correctly assigned. "
                f"(confidence mismatch: {prob:.1%})"
            )
