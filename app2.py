...
st.markdown(
    ...
    .metric-label { font-size: 0.85rem; color: #aab3c5; margin-bottom: 0.2rem; }
    .metric-value { font-size: 1.85rem; font-weight: 700; line-height: 1.1; margin-bottom: 0.2rem; }
    .metric-sub { font-size: 0.84rem; color: #8f98aa; }
    .section-card {
    ...
    .risk-title { font-size: 1.04rem; font-weight: 700; margin-bottom: 0.8rem; }
    ...
    .bar-fill { height: 100%; border-radius: 999px; background: linear-gradient(90deg, #e05c2a, #f5a35e); }
    .bar-fill-blue { height: 100%; border-radius: 999px; background: linear-gradient(90deg, #5b8fc9, #7dc1ff); }
    </style>
    """,
...
def make_input(row):
    ...
    return f"[SUBJ] {row['Ticket_Subject']} [BODY] {row['Ticket_Description']} | cat:{row['Issue_Category']} | ch:{row.get('Ticket_Channel', 'Unknown')} | rt:{rt_tier(rt)} | pri:{row['Priority_Level']}"
...
def predict(texts, tok, model, thr):
    ...
    for i in range(0, len(texts), 32):
        enc = tok(texts[i:i + 32], truncation=True, padding="max_length", max_length=256, return_tensors="pt")
...
def make_dossier(row, prob):
    ...
    exp = RT_BENCH.get((row["Issue_Category"], row["Priority_Level"]), 40.0)
    r = rt / max(exp, 1)
    ev.append({"signal": "resolution_time", "value": f"{rt:.0f}h", "expected": f"~{exp:.0f}h", "source_field": "Resolution_Time_Hours", "weight": round(0.25 if r < 0.4 else 0.20 if r > 2.5 else 0.05, 3)})
    cs = CAT_SEV.get(row["Issue_Category"], {"exp": "Medium", "w": 0.05, "note": ""})
    ev.append({"signal": "category_baseline", "value": row["Issue_Category"], "source_field": "Issue_Category", "weight": round(abs(cs["w"]), 3)})
    ev.append({"signal": "satisfaction_score", "value": str(int(row["Satisfaction_Score"])), "source_field": "Satisfaction_Score", "weight": round(0.18 if int(row["Satisfaction_Score"]) <= 2 else 0.02, 3)})
    ev = sorted(ev, key=lambda x: abs(x.get("weight", 0)), reverse=True)
    a = PMAP.get(row["Priority_Level"], 1)
    ii = PMAP.get(inf, 1)
    d = ii - a
    delta = f"+{d} (under-prioritised by {d} level{'s' if d > 1 else ''})" if d > 0 else f"{d} (over-prioritised by {abs(d)} level{'s' if abs(d) > 1 else ''})" if d < 0 else "0 (borderline)"
    ...
    s2 = f"Escalation indicators ({', '.join(repr(e['value']) for e in kp[:2])}) signal higher severity." if mtype == "Hidden Crisis" and kp else f"Low-severity indicators ({', '.join(repr(e['value']) for e in kn[:2])}) suggest over-prioritisation." if mtype == "False Alarm" and kn else "Semantic patterns and metadata signal priority mismatch."
    s3 = f"RT={rt:.0f}h with sat={int(row['Satisfaction_Score'])}/5 supports under-prioritisation." if mtype == "Hidden Crisis" else f"RT={rt:.0f}h with sat={int(row['Satisfaction_Score'])}/5 consistent with over-triage."
    return {"ticket_id": str(row.get("Ticket_ID", "")), "assigned_priority": row["Priority_Level"], "inferred_severity": inf, "mismatch_type": mtype, "severity_delta": delta, "confidence": round(float(prob), 4), "feature_evidence": ev, "constraint_analysis": f"{s1} {s2} {s3}"}

def zplot(fig, h=380):
    fig.update_layout(autosize=True, height=h, margin=dict(t=40, b=20, l=20, r=20))
...
def card_html(title, value, sub="", accent="orange"):
    ...
    return f"""<div class="card"><div class="card-topline"><div class="{accent_class}">{title}</div></div><div class="metric-value">{value}</div><div class="metric-sub">{sub}</div></div>"""
...
def rank_bars(series, color="orange", max_items=8):
    ...
    for name, val in s.items():
        ...
        st.markdown(f"""<div style="margin-bottom:0.75rem;"><div style="display:flex;justify-content:space-between;gap:0.75rem;margin-bottom:0.25rem;"><div class="rank-name">{name}</div><div class="small-muted">{float(val):.1f}</div></div><div class="bar-wrap"><div class="{bar_class}" style="width:{pct:.1f}%"></div></div></div>""", unsafe_allow_html=True)
...
def single_ticket_decision(row, prob, thr):
    score = dir_score(row)
    if score >= 0.35:
        final_pred = 1
        inferred, mtype = get_verdict(row, max(prob, thr))
        ...
        final_pred = 1 if prob >= thr else 0
        if final_pred == 1:
            inferred, mtype = get_verdict(row, prob)
        else:
            inferred, mtype = row["Priority_Level"], "Consistent"
    return {"final_pred": final_pred, "score": score, "inferred": inferred, "mtype": mtype, "confidence": float(max(prob, min(abs(score), 1.0))) if final_pred == 1 else float(1 - prob)}

tok, model, THR = load_model()
model_ready = tok is not None

st.markdown("""<div class="hero"><h1 style="margin:0;color:#ffeadf;">🔍 SIA — Support Integrity Auditor</h1><p style="margin:0.35rem 0 0 0;color:#d4d9e3;">Detect priority mismatches in CRM support tickets · MARS Open Projects 2026</p></div>""", unsafe_allow_html=True)
...
with tab1:
    ...
    with c2:
        desc = st.text_area("Ticket Description", height=130, value="I have been locked out of my account since yesterday. I cannot access any of my data and payment is overdue.")
    ...
    if st.button("🔍 Analyse Ticket", type="primary"):
        row = {"Ticket_ID": "SINGLE-001", "Ticket_Subject": subject, "Ticket_Description": desc, "Issue_Category": category, "Ticket_Channel": channel, "Priority_Level": priority, "Resolution_Time_Hours": rt, "Satisfaction_Score": sat}
        txt = make_input(row)
        probs, _ = predict([txt], tok, model, THR)
        prob = float(probs[0])
        decision = single_ticket_decision(row, prob, THR)
        final_pred = decision["final_pred"]
        score = decision["score"]
        inferred = decision["inferred"]
        mtype = decision["mtype"]
        conf = decision["confidence"]
        st.divider()
        col_a, col_b, col_c = st.columns(3)
        if final_pred == 1:
            col_a.metric("Verdict", f"🚨 {mtype.upper()}")
            col_b.metric("Confidence", f"{conf:.1%}", f"Model prob: {prob:.1%}")
            col_c.metric("Inferred Severity", inferred, f"Assigned: {priority}")
            if mtype == "Hidden Crisis":
                st.error(f"**{mtype}** — Assigned `{priority}` but model/business signals infer `{inferred}`.")
            else:
                st.warning(f"**{mtype}** — Assigned `{priority}` appears too high for the ticket signals.")
            with st.expander("📋 Evidence Dossier", expanded=True):
                st.json(make_dossier(row, prob))
        else:
            col_a.metric("Verdict", "✅ CONSISTENT")
            col_b.metric("Confidence (Mismatch)", f"{(1-prob):.1%}", f"Direction score: {score:.2f}")
            col_c.metric("Priority", priority)
            st.success(f"Priority **{priority}** appears correctly assigned. (mismatch confidence: {prob:.1%})")
...
with tab2:
    ...
    if batch_df is None:
        ...
        with k1: st.markdown(card_html("Hidden Crises", f"{len(hidden):,}", "Under-prioritised tickets", accent="orange"), unsafe_allow_html=True)
        with k2: st.markdown(card_html("False Alarms", f"{len(falsea):,}", "Over-prioritised tickets", accent="blue"), unsafe_allow_html=True)
        with k3: st.markdown(card_html("Flagged Total", f"{len(flagged):,}", f"of {len(batch_df):,} tickets", accent="orange"), unsafe_allow_html=True)
        with k4: st.markdown(card_html("Top Category", top_cat, "Highest mismatch concentration", accent="blue"), unsafe_allow_html=True)
        with k5: st.markdown(card_html("Avg Confidence", f"{avg_conf:.1f}%", "Mismatch probability", accent="orange"), unsafe_allow_html=True)
        ...
        if hidden.empty:
            ...
            for i, (_, r) in enumerate(top_hidden.iterrows()):
                with cols[i % 2]:
                    ...
                    st.markdown(f"""<div class="card"><div class="card-topline"><div class="pill">Hidden Crisis</div><div class="pill pill-blue">{float(r.get('prob', 0))*100:.1f}%</div></div><div class="ticket-id">{r['Ticket_ID']}</div><div class="ticket-meta">Assigned: <b>{r.get('Priority_Level', '—')}</b><br/>Inferred: <b>{r.get('inferred_severity', r.get('inferred_sev', '—'))}</b><br/>Category: <b>{r.get('Issue_Category', '—')}</b>{"<br/>Channel: <b>" + str(r['Ticket_Channel']) + "</b>" if 'Ticket_Channel' in r.index else ""}</div><div class="ticket-desc">{desc_txt}</div></div>""", unsafe_allow_html=True)
        ...
        if falsea.empty:
            ...
            for i, (_, r) in enumerate(top_false.iterrows()):
                with cols[i % 2]:
                    desc_txt = str(r["Ticket_Description"])[:220] if "Ticket_Description" in r.index else "Description unavailable in this CSV."
                    st.markdown(f"""<div class="card"><div class="card-topline"><div class="pill pill-blue">False Alarm</div><div class="pill">{float(r.get('prob', 0))*100:.1f}%</div></div><div class="ticket-id">{r['Ticket_ID']}</div><div class="ticket-meta">Assigned: <b>{r.get('Priority_Level', '—')}</b><br/>Inferred: <b>{r.get('inferred_severity', r.get('inferred_sev', '—'))}</b><br/>Category: <b>{r.get('Issue_Category', '—')}</b>{"<br/>Channel: <b>" + str(r['Ticket_Channel']) + "</b>" if 'Ticket_Channel' in r.index else ""}</div><div class="ticket-desc">{desc_txt}</div></div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if flagged.empty or "Ticket_Channel" not in flagged.columns:
            ...
            hotspot = flagged.groupby(["Issue_Category", "Ticket_Channel"], dropna=False)["predicted"].mean().reset_index(name="Mismatch Rate")
            hotspot["Mismatch Rate (%)"] = hotspot["Mismatch Rate"] * 100
            fig = px.density_heatmap(hotspot, x="Ticket_Channel", y="Issue_Category", z="Mismatch Rate (%)", color_continuous_scale="RdYlGn_r", title="Mismatch Rate by Category and Channel")
            zplot(fig, h=520)
        st.markdown("### Investigation Queue")
        if flagged.empty:
            st.info("No tickets in the investigation queue.")
        else:
            queue_cols = [c for c in ["Ticket_ID", "Priority_Level", "Issue_Category", "Ticket_Channel", "prob", "mtype", "inferred_severity", "inferred_sev", "verdict"] if c in flagged.columns]
            ...
            chosen_id = st.selectbox("Open a ticket for full evidence", options=flagged["Ticket_ID"].astype(str).tolist())
        ...
        summary = pd.DataFrame({"Metric": ["Total Tickets", "Flagged Mismatches", "Hidden Crises", "False Alarms", "Top Category", "Top Channel"], "Value": [f"{len(batch_df):,}", f"{len(flagged):,}", f"{len(hidden):,}", f"{len(falsea):,}", top_cat, top_ch if top_ch != "-" else "Not available"]})
        st.dataframe(summary, use_container_width=True, hide_index=True)

with tab3:
    st.subheader("Executive Dashboard")
    src = st.session_state.get("batch_df")
    if src is None:
        src, _, _ = load_batch_data()
        if src is not None:
            src = normalize_source_columns(src)
            st.session_state["batch_df"] = src.copy()
    if src is None:
        ...
        with left:
            ...
            if "mtype" in src.columns:
                ...
                if len(vc) > 0:
                    fig = px.bar(x=vc.index, y=vc.values, color=vc.index, color_discrete_map={"Hidden Crisis": "#e05c2a", "False Alarm": "#5b8fc9", "Consistent": "#5e9e6e"}, labels={"x": "", "y": "Count"})
        ...
        with right:
            ...
            if "predicted" in src.columns and "Issue_Category" in src.columns:
                ...
                fig = px.bar(x=grp.values, y=grp.index, orientation="h", color=grp.values, color_continuous_scale="RdYlGn_r", labels={"x": "Mismatch Rate (%)", "y": ""})
        ...
        with c1:
            ...
            if "delta" in src.columns and "Ticket_Channel" in src.columns:
                pivot = src.pivot_table(values="delta", index="Issue_Category", columns="Ticket_Channel", aggfunc="mean").fillna(0)
                fig = px.imshow(pivot, color_continuous_scale="RdBu_r", color_continuous_midpoint=0, text_auto=".2f", aspect="auto", labels={"color": "Mean Δ", "x": "Channel", "y": "Category"})
            ...
            if dos_path.exists():
                try:
                    ...
                    if sig_counts:
                        ...
                        fig = px.bar(sc_df, x="Count", y="Signal Type", orientation="h", color="Count", color_continuous_scale="Blues")
        ...
        if "predicted" in src.columns and "Ticket_Channel" in src.columns:
            heat = src.groupby(["Issue_Category", "Ticket_Channel"], dropna=False)["predicted"].mean().reset_index(name="Mismatch Rate")
            if not heat.empty:
                heat["Mismatch Rate (%)"] = heat["Mismatch Rate"] * 100
                fig = px.density_heatmap(heat, x="Ticket_Channel", y="Issue_Category", z="Mismatch Rate (%)", color_continuous_scale="RdYlGn_r")
                zplot(fig, h=520)
        else:
